...
async function getData() {
  tableData.value = [];
  tempDate.value = [];
  const result = (await api.getCollectionOfUser(proxy.$route.query.id)) as any;
  for (let item of result.data) {
    const result = await api.getSongOfId(item.songId) as any;
...
async function confirm() {
  const result = (await api.deleteCollection(proxy.$route.query.id, idx.value)) as ResponseBody;
