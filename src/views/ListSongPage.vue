...
async function getData() {
  tableData.value = [];
  tempDate.value = [];
  const result = (await api.getListSongOfSongId(proxy.$route.query.id)) as ResponseBody;
  for (let item of result.data) {
    const result = await api.getSongOfId(item.songId) as ResponseBody;
...
async function saveSong() {
  ...
  const result = (await api.getSongOfSingerName(id)) as ResponseBody;
...
async function confirm() {
  const result = await api.deleteListSong(idx.value) as ResponseBody;
  (proxy as any).$message({
    message: result.message,
    type: result.type,
  });
