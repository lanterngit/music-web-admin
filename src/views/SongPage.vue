...
async function getData() {
  tableData.value = [];
  tempDate.value = [];
  const result = (await api.getSongOfSingerId(singerId.value)) as ResponseBody;
...
function addSong() {
  ...
  req.open("post", api.attachImageUrl(`/song/add`), false);
...
async function confirm() {
  const result = (await api.deleteSong(idx.value)) as ResponseBody;
  (proxy as any).$message({
    message: result.message,
    type: result.type,
  });
