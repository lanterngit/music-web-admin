...
async function getData() {
  tableData.value = [];
  tempDate.value = [];
  const result = (await api.getSongList()) as ResponseBody;
...
function uploadUrl(id) {
  return api.attachImageUrl(`/songList/img/update?id=${id}`);
}
...
async function addsongList() {
  ...
  const result = (await api.setSongList({title, introduction, style})) as ResponseBody;
  (proxy as any).$message({
...
async function saveEdit() {
  ...
  const result = (await api.updateSongListMsg({id, title, introduction, style})) as ResponseBody;
  (proxy as any).$message({
    message: result.message,
    type: result.type,
  });
...
async function confirm() {
  const result = await api.deleteSongList(idx.value) as ResponseBody;
  (proxy as any).$message({
    message: result.message,
    type: result.type,
  });
